...
def test_escape_html_system_reminder(viz):
    out = viz.escape_html_preserve_structure("<system-reminder>Aviso del sistema</system-reminder>")
    assert 'class="system-reminder"' in out
    assert "Aviso del sistema" in out
    assert "system-reminder&gt;" not in out  # las etiquetas no quedan crudas


def test_escape_html_system_reminder_escapa_contenido(viz):
    # El contenido del reminder sigue escapado (sin inyección de HTML).
    out = viz.escape_html_preserve_structure("<system-reminder><script>x</script></system-reminder>")
    assert "<script>x</script>" not in out
    assert 'class="system-reminder"' in out


def test_escape_html_request_interrupted(viz):
    out = viz.escape_html_preserve_structure("[Request interrupted by user]")
    assert 'class="request-interrupted"' in out
    assert "[Request interrupted by user]" in out


def test_is_tool_use_interrupted_message(viz):
    # Gemelo de un rechazo de tool → True (se oculta; ya hay bloque [REJECTED]).
    assert viz.is_tool_use_interrupted_message(
        [{"type": "text", "text": "[Request interrupted by user for tool use]"}]) is True
    # La interrupción simple NO se oculta (se muestra en rojo).
    assert viz.is_tool_use_interrupted_message(
        [{"type": "text", "text": "[Request interrupted by user]"}]) is False
    # Texto normal → False.
    assert viz.is_tool_use_interrupted_message([{"type": "text", "text": "hola"}]) is False


def test_escape_html_marker_mezclado_con_texto(viz):
    # Embebido con contenido real: el texto se mantiene, el marker se formatea.
    out = viz.escape_html_preserve_structure("Hola mundo [Request interrupted by user]")
...
def test_is_image_source_message(viz):
    # Solo referencias [Image: source: ...] → True (se oculta).
    ref = [{"type": "text", "text": "[Image: source: C:\\Users\\x\\.claude\\image-cache\\a\\1.png]"}]
    assert viz.is_image_source_message(ref) is True
    # Varias referencias en el mismo mensaje → True.
    refs = [
        {"type": "text", "text": "[Image: source: C:\\x\\1.png]"},
        {"type": "text", "text": "[Image: source: C:\\x\\2.png]"},
    ]
    assert viz.is_image_source_message(refs) is True
    # Con la imagen real presente → False (no se oculta el mensaje de la imagen).
    con_imagen = [
        {"type": "text", "text": "[Image #1]"},
        {"type": "image", "source": {"type": "base64", "data": "x"}},
    ]
    assert viz.is_image_source_message(con_imagen) is False
    # Texto normal → False.
    assert viz.is_image_source_message([{"type": "text", "text": "hola"}]) is False
...
# ====== extract_ask_data (structured toolUseResult, with text fallback) ======

def test_extract_ask_data_estructura_opcion_y_libre(viz):
    """Desde toolUseResult: marca la opción elegida y reconoce la respuesta libre
    ('Other') cuando no casa con ningún label."""
    msg = {"toolUseResult": {
        "questions": [
            {"question": "¿Color?", "header": "Tono", "multiSelect": False,
             "options": [{"label": "Azul", "description": "el azul"},
                         {"label": "Rojo", "description": "el rojo"}]},
            {"question": "¿Modelo?", "header": "IA", "multiSelect": False,
             "options": [{"label": "GPT", "description": "x"}]},
        ],
        "answers": {"¿Color?": "Azul", "¿Modelo?": "Otro que escribí"},
        "annotations": {"¿Modelo?": {"notes": "Otro que escribí"}}}}
    data = viz.extract_ask_data({"type": "tool_result"}, msg)
    assert data is not None and len(data) == 2
    q1 = data[0]
    assert q1["question"] == "¿Color?" and q1["header"] == "Tono"
    sel = [o for o in q1["options"] if o["selected"]]
    assert len(sel) == 1 and sel[0]["label"] == "Azul"
    assert q1["answer_is_free"] is False
    q2 = data[1]
    assert q2["answer_is_free"] is True and q2["answer"] == "Otro que escribí"
    assert not any(o["selected"] for o in q2["options"])


def test_extract_ask_data_multiselect_varias(viz):
    """multiSelect con respuesta coma-unida: marca todas las opciones elegidas."""
    msg = {"toolUseResult": {
        "questions": [{"question": "¿Cuáles?", "multiSelect": True,
                       "options": [{"label": "A"}, {"label": "B"}, {"label": "C"}]}],
        "answers": {"¿Cuáles?": "A, C"}}}
    data = viz.extract_ask_data({"type": "tool_result"}, msg)
    sel = {o["label"] for o in data[0]["options"] if o["selected"]}
    assert sel == {"A", "C"} and data[0]["answer_is_free"] is False


def test_extract_ask_data_fallback_texto(viz):
    """Sin toolUseResult, cae al parseo del texto (mensajes antiguos): items sin opciones."""
    item = {"type": "tool_result",
            "content": 'User has answered your questions: "¿Q?" = "A". You can now continue.'}
    data = viz.extract_ask_data(item, {})
    assert data and data[0]["question"] == "¿Q?" and data[0]["answer"] == "A"
    assert data[0]["options"] == []


def test_extract_ask_data_no_es_ask(viz):
    """Un tool_result normal (no AskUserQuestion) devuelve None."""
    assert viz.extract_ask_data({"type": "tool_result", "content": "salida normal"}, {}) is None
