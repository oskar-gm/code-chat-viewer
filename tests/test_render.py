...
def test_render_ask_result_block_opciones_seleccionada(viz):
    """Con opciones, la elegida se resalta (ask-opt-sel), el header sale como chip
    y las no elegidas también se listan."""
    items = [{"question": "¿Color?", "header": "Tono", "multiSelect": False,
              "options": [{"label": "Azul", "description": "el azul", "selected": True},
                          {"label": "Rojo", "description": "el rojo", "selected": False}],
              "answer": "Azul", "answer_is_free": False, "notes": "", "markdown": ""}]
    html = viz.render_ask_result_block(items, "t", "u")
    assert "ask-opt-sel" in html
    assert "ask-chip" in html and "Tono" in html
    assert "Azul" in html and "Rojo" in html


def test_render_ask_result_block_respuesta_libre(viz):
    """Respuesta libre ('Other'): se muestra el texto escrito, no '(no option selected)'."""
    items = [{"question": "¿Modelo?", "header": "IA", "multiSelect": False,
              "options": [{"label": "GPT", "description": "x", "selected": False}],
              "answer": "Otro que escribí", "answer_is_free": True, "notes": "", "markdown": ""}]
    html = viz.render_ask_result_block(items, "t", "u")
    assert "ask-free" in html and "Otro que escribí" in html


# ====== render_compact_block ======

def test_render_compact_block_navegable(viz):
    """El bloque compact emite data-msg-uuid para que el botón Go del rewind
    pueda navegar a él (antes quedaba sin ancla y el Go salía inerte)."""
    html = viz.render_compact_block({"summary_text": "un resumen", "uuid": "abc-123-uuid",
                                     "time_str": "10:00"})
    assert 'data-msg-uuid="abc-123-uuid"' in html
...
# ====== format_content_item: image (opción A — enlace data URI) ======

def test_render_image_genera_enlace(viz):
    item = {"type": "image", "source": {"type": "base64", "media_type": "image/png", "data": "aGVsbG8="}}
    html = viz.format_content_item(item)
    assert "msg-image-link" in html
    # base64 en data-img (no en href data:, que el navegador bloquearía al abrir).
    assert 'data-img="aGVsbG8="' in html
    assert 'data-media="image/png"' in html
    assert "openImage" in html
    assert "Open image" in html


def test_render_image_no_pinta_inline(viz):
    # Opción A: enlace, NO etiqueta <img> — la imagen no se renderiza inline.
    item = {"type": "image", "source": {"type": "base64", "media_type": "image/png", "data": "aGk="}}
    html = viz.format_content_item(item)
    assert "<img" not in html
    assert "<a " in html


def test_render_image_muestra_peso(viz):
    data = "A" * 4096  # ~3 KB decodificado
    item = {"type": "image", "source": {"type": "base64", "media_type": "image/png", "data": data}}
    html = viz.format_content_item(item)
    assert "KB)" in html


def test_render_image_sin_data_placeholder(viz):
    html = viz.format_content_item({"type": "image", "source": {}})
